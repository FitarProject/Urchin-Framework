...
class TaskThread(QThread):
    ...
    def __init__(self, parent, task_input_dict, func_detatil, thread_log):
...
'''
    ...
    def exec_command(self, command):
        try:
            ...
            self.pause = psutil.Process(self.pid)
            ...
            read_stdout = TextReadLineThread(self.process.stdout.readline, log_warp(self.to_out))
            read_stderr = TextReadLineThread(self.process.stderr.readline, log_warp(self.to_err))
            read_stdout.start()
            read_stderr.start()
            ...
            # print(3)
            # # 创建队列读取函数并弹出日志
            # self.thread_queue_out = ReadQueueThread(self.start_RealSignal_out, self.stop_RealSignal_out, self.queue_out)
...
'''
            print(1)
            # 动态信号
            start_RealSignal_out = threading.Event()
            stop_RealSignal_out = threading.Event()
            start_RealSignal_err = threading.Event()
            stop_RealSignal_err = threading.Event()
            # 创建队列并执行
            queue_out = Queue()
            queue_err = Queue()
            print(2)
            ...
            print(3)
            # 创建队列读取函数并弹出日志
            thread_queue_out = ReadQueueThread(start_RealSignal_out, stop_RealSignal_out, queue_out)
            thread_queue_err = ReadQueueThread(start_RealSignal_err, stop_RealSignal_err, queue_out)
            thread_queue_out.trigger.connect(self.to_out)
            thread_queue_err.trigger.connect(self.to_err)
            print(4)
            stop_RealSignal_out.set()
            start_RealSignal_out.clear()
            stop_RealSignal_err.set()
            start_RealSignal_err.clear()
            thread_queue_out.start()
            thread_queue_err.start()
            thread_queue_out.exit()
            thread_queue_err.exit()
            print(5)
